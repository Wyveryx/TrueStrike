------------------------------------------------------------------------
-- TrueStrike Battle Text - Configuration Tab Definitions
...
function TSBT.BuildTab_General()
    return {
        ...
        args = {
            ...
            enabled = {
                type = "toggle",
                name = "Enable TrueStrike",
                desc = "Master switch to enable or disable all TrueStrike output.",
                width = "full",
                order = 2,
                get = function()
                    return TSBT.db.profile.general.enabled
                end,
                set = function(_, val)
                    ...
                    if TSBT.Core then
                        if val and TSBT.Core.Enable then
                            TSBT.Core:Enable()
                        end
                        if (not val) and TSBT.Core.Disable then
                            TSBT.Core:Disable()
                        end
                    end
                    ...
                    if TSBT.Parser then
                        if val then
                            if TSBT.Parser.CombatLog and
                                TSBT.Parser.CombatLog.Enable then
                                TSBT.Parser.CombatLog:Enable()
                            end
                            if TSBT.Parser.Cooldowns and
                                TSBT.Parser.Cooldowns.Enable then
                                TSBT.Parser.Cooldowns:Enable()
                            end
                        else
                            if TSBT.Parser.Cooldowns and
                                TSBT.Parser.Cooldowns.Disable then
                                TSBT.Parser.Cooldowns:Disable()
                            end
                            if TSBT.Parser.CombatLog and
                                TSBT.Parser.CombatLog.Disable then
            ...
            combatOnly = {
                ...
                desc = "Only display TrueStrike output while you are in combat.",
                width = "full",
                order = 3,
                get = function()
                    return TSBT.db.profile.general.combatOnly
                end,
                set = function(_, val)
            ...
            disableBlizzardFCT = {
                ...
                order = 4,
                get = function()
                    return TSBT.db.profile.general.disableBlizzardFCT
                end,
                set = function(_, val)
            ...
            headerFont = {type = "header", name = "Master Font", order = 10},
            fontFace = {
                type = "select",
                name = "Font Face",
                desc = "Global font used for all TrueStrike combat text.",
                order = 11,
                values = function()
                    return TSBT.BuildFontDropdown()
                end,
                get = function()
                    return TSBT.db.profile.general.font.face
                end,
                set = function(_, val)
            ...
            fontSize = {
                type = "range",
                name = "Font Size",
                desc = "Base size for TrueStrike combat text numbers.",
                order = 12,
                min = TSBT.FONT_SIZE_MIN,
                max = TSBT.FONT_SIZE_MAX,
                step = 1,
                get = function()
                    return TSBT.db.profile.general.font.size
                end,
                set = function(_, val)
            ...
            fontOutline = {
                ...
                get = function()
                    return TSBT.db.profile.general.font.outline
                end,
                set = function(_, val)
            ...
            fontAlpha = {
                type = "range",
                name = "Text Opacity",
                desc = "Global transparency for TrueStrike text.",
                order = 14,
                min = TSBT.ALPHA_MIN,
                max = TSBT.ALPHA_MAX,
                step = 0.05,
                isPercent = true,
                get = function()
                    return TSBT.db.profile.general.font.alpha
                end,
                set = function(_, val)
            ...
            resetDefaults = {
                ...
                func = function()
                    ...
                    if TSBT.db and TSBT.db.profile and TSBT.db.profile.cooldowns and
                        TSBT.db.profile.cooldowns.tracked then
...
local function GetFallbackScrollAreaName()
    ...
    if TSBT and TSBT.db and TSBT.db.profile and TSBT.db.profile.scrollAreas then
        if TSBT.db.profile.scrollAreas["Incoming"] then return "Incoming" end
        for name in pairs(TSBT.db.profile.scrollAreas) do return name end
    end
    return nil
end
    ...
    return {
        ...
        args = {
            ...
            rowSelect = {
                ...
                args = {
                    selectArea = {
                        ...
                        width = "double",
                        values = function()
                            return TSBT.GetScrollAreaNames()
                        end,
                        get = function()
                            return EnsureSelectedScrollArea()
                        end,
                        set = function(_, val)
                    ...
                    deleteArea = {
                        ...
                        width = "normal",
                        disabled = function()
                            return not EnsureSelectedScrollArea()
                        end,
                        func = function()
                            ...
                            local count = 0
                            for _ in pairs(TSBT.db.profile.scrollAreas) do
                                count = count + 1
                            end
                            ...
                            if count <= 1 then
                                ...
                                StaticPopup_Show(
                                    "TRUESTRIKE_DELETE_LAST_SCROLLAREA", sel,
                                    nil, sel)
                                return
                            end

                            StaticPopup_Show("TRUESTRIKE_DELETE_SCROLLAREA",
                                             sel, nil, sel)
                        end
                    }
                }
                ...
                args = {
                    ...
                    applyRename = {
                        ...
                        disabled = function()
                            local sel = EnsureSelectedScrollArea()
                            local newName =
                                strtrim(renameScrollAreaBuffer or "")
                            if not sel or newName == "" or newName == sel then
                                return true
                            end
                        ...
                        func = function()
                            ...
                            local newName =
                                strtrim(renameScrollAreaBuffer or "")
                            if not oldName or newName == "" or newName ==
                                oldName then return end
                            ...
                            TSBT.db.profile.scrollAreas[newName] = TSBT.db
                                                                       .profile
                                                                       .scrollAreas[oldName]
                            ...
                            if TSBT.IsScrollAreasUnlocked and
                                TSBT.IsScrollAreasUnlocked() and
                                TSBT.RefreshScrollAreaFrames then
            ...
            rowCreate = {
                ...
                args = {
                    createAreaName = {
                        ...
                        width = "full",
                        get = function()
                            return createScrollAreaBuffer or ""
                        end,
                        set = function(_, val)
                            val = strtrim(val or "")
                            ...
                            TSBT.db.profile.scrollAreas[val] = {
                                ...
                                font = {
                                    ...
                                    face = TSBT.db.profile.general.font.face,
                                    size = TSBT.db.profile.general.font.size,
                                    outline = TSBT.db.profile.general.font
                                        .outline,
                                    alpha = TSBT.db.profile.general.font.alpha
                                }
                            }
            ...
            unlockAreas = {
                ...
                name = function()
                    if TSBT.IsScrollAreasUnlocked and
                        TSBT.IsScrollAreasUnlocked() then
            ...
            areaFontFace = {
                ...
                hidden = function()
                    if not selectedScrollArea then
                        return true
                    end
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    return not area or not area.font or area.font.useGlobal ~=
                               false
                end,
                get = function()
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    return area and area.font and area.font.face or
                               TSBT.db.profile.general.font.face
                end,
                set = function(_, val)
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    if not area then return end
            ...
            areaFontSize = {
                ...
                get = function()
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    return area and area.font and area.font.size or
                               TSBT.db.profile.general.font.size
                end,
                set = function(_, val)
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    if not area then return end
            ...
            areaFontOutline = {
                ...
                get = function()
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    return area and area.font and area.font.outline or
                               TSBT.db.profile.general.font.outline
                end,
                set = function(_, val)
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    if not area then return end
            ...
            areaFontAlpha = {
                ...
                get = function()
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    return area and area.font and area.font.alpha or
                               TSBT.db.profile.general.font.alpha
                end,
                set = function(_, val)
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    if not area then return end
            ...
            animSpeed = {
                ...
                hidden = function()
                    ...
                    local area = TSBT.db.profile.scrollAreas[selectedScrollArea]
                    if area and area.animation == "Static" then
                        return true
                    end
                    return false
                end,
            ...
            testAnimation = {
                ...
                disabled = function()
                    ...
                    if TSBT.IsScrollAreasUnlocked and
                        not TSBT.IsScrollAreasUnlocked() then
                        return true
                    end
                    return false
                end,
...
function TSBT.BuildTab_Incoming()
    local function cap()
        return TSBT.Core and TSBT.Core.IncomingProbe and
                   TSBT.Core.IncomingProbe.cap
    end
    ...
    local function reportLine()
        ...
        return
            ("Probe: source=%s | flags=%s | school=%s | periodic=%s | buffer=%d/%d"):format(
                r.source, r.hasFlagText, r.hasSchool, r.hasPeriodic,
                r.bufferCount, r.bufferMax)
    end
    ...
    return {
        ...
        args = {
            ...
            damageEnabled = {
                ...
                order = 2,
                get = function()
                    return TSBT.db.profile.incoming.damage.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.damage.enabled = val
                end
            },
            damageScrollArea = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.incoming.damage.scrollArea
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.damage.scrollArea = val
                end
            },
            damageShowFlags = {
                type = "toggle",
                ...
                get = function()
                    return TSBT.db.profile.incoming.damage.showFlags
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.damage.showFlags = val
                end
            },
            damageMinThreshold = {
                type = "range",
                ...
                get = function()
                    return TSBT.db.profile.incoming.damage.minThreshold
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.damage.minThreshold = val
                end
            },
            ...
            healingEnabled = {
                ...
                order = 11,
                get = function()
                    return TSBT.db.profile.incoming.healing.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.healing.enabled = val
                end
            },
            healingScrollArea = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.incoming.healing.scrollArea
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.healing.scrollArea = val
                end
            },
            healingShowHoTs = {
                type = "toggle",
                ...
                get = function()
                    return TSBT.db.profile.incoming.healing.showHoTTicks
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.healing.showHoTTicks = val
                end
            },
            healingShowSpellInfo = {
                ...
                order = 13.5,
                get = function()
                    return TSBT.db.profile.incoming.healing.showSpellInfo
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.healing.showSpellInfo = val
                end
            },
            ...
            hotSpellIDInput = {
                ...
                set = function(_, val)
                    ...
                    if TSBT.db.profile.incoming.healing.hotSpellIDs[spellID] then
                        Addon:Print(spellName .. " (ID: " .. spellID ..
                                        ") is already tracked.")
                        return
                    end

                    TSBT.db.profile.incoming.healing.hotSpellIDs[spellID] = true
                    Addon:Print("Added HoT: " .. spellName .. " (ID: " ..
                                    spellID .. ")")
                    LibStub("AceConfigRegistry-3.0"):NotifyChange("TrueStrike")
                end
            },
            ...
            healingMinThreshold = {
                ...
                disabled = true, -- Always disabled in WoW 12.0
                get = function()
                    return TSBT.db.profile.incoming.healing.minThreshold
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.healing.minThreshold = val
                end
            },
            ...
            useSchoolColors = {
                ...
                get = function()
                    return TSBT.db.profile.incoming.useSchoolColors
                end,
                set = function(_, val)
                    TSBT.db.profile.incoming.useSchoolColors = val
                end
            },
            customColor = {
                type = "color",
            ...
            probePrint = {
                ...
                func = function()
                    local p = TSBT.Core and TSBT.Core.IncomingProbe
                    if p and p.PrintCapabilityReport then
                        p:PrintCapabilityReport()
                    end
                end
            }
    ...
    return {
        ...
        args = {
            ...
            damageEnabled = {
                ...
                order = 2,
                get = function()
                    return TSBT.db.profile.outgoing.damage.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.damage.enabled = val
                end
            },
            showSpellNames = {
                type = "toggle",
                ...
                order = 3,
                get = function()
                    return TSBT.db.profile.outgoing.showSpellNames
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.showSpellNames = val
                end
            },
            damageScrollArea = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.outgoing.damage.scrollArea
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.damage.scrollArea = val
                end
            },
            showTargets = {
                type = "toggle",
                ...
                order = 5,
                get = function()
                    return TSBT.db.profile.outgoing.damage.showTargets
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.damage.showTargets = val
                end
            },
            autoAttackMode = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.outgoing.damage.autoAttackMode
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.damage.autoAttackMode = val
                end
            },
            damageMinThreshold = {
                type = "range",
                ...
                step = 50,
                get = function()
                    return TSBT.db.profile.outgoing.damage.minThreshold
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.damage.minThreshold = val
                end
            },
            ...
            healingEnabled = {
                ...
                order = 11,
                get = function()
                    return TSBT.db.profile.outgoing.healing.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.healing.enabled = val
                end
            },
            healingScrollArea = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.outgoing.healing.scrollArea
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.healing.scrollArea = val
                end
            },
            showOverheal = {
                type = "toggle",
                ...
                order = 13,
                get = function()
                    return TSBT.db.profile.outgoing.healing.showOverheal
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.healing.showOverheal = val
                end
            },
            healingMinThreshold = {
                type = "range",
                ...
                step = 50,
                get = function()
                    return TSBT.db.profile.outgoing.healing.minThreshold
                end,
                set = function(_, val)
                    TSBT.db.profile.outgoing.healing.minThreshold = val
                end
            },
...
function TSBT.BuildTab_Pets()
    return {
        ...
        order = 5,
        args = {
            headerPets = {type = "header", name = "Pet Damage", order = 1},
            enabled = {
                type = "toggle",
                ...
                order = 2,
                get = function()
                    return TSBT.db.profile.pets.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.pets.enabled = val
                end
            },
            scrollArea = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.pets.scrollArea
                end,
                set = function(_, val)
                    TSBT.db.profile.pets.scrollArea = val
                end
            },
            aggregation = {
                type = "select",
                ...
                get = function()
                    return TSBT.db.profile.pets.aggregation
                end,
                set = function(_, val)
                    TSBT.db.profile.pets.aggregation = val
                end
            },
            minThreshold = {
                type = "range",
                ...
                step = 50,
                get = function()
                    return TSBT.db.profile.pets.minThreshold
                end,
                set = function(_, val)
                    TSBT.db.profile.pets.minThreshold = val
                end
            },
...
function TSBT.BuildTab_SpamControl()
    return {
        ...
        args = {
            ...
            -- Merging
            ----------------------------------------------------------------
            headerMerge = {type = "header", name = "Spell Merging", order = 1},
            mergeDesc = {
                type = "description",
            ...
            mergeEnabled = {
                ...
                order = 3,
                get = function()
                    return TSBT.db.profile.spamControl.merging.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.merging.enabled = val
                end
            },
            mergeWindow = {
                type = "range",
                ...
                step = 0.1,
                disabled = function()
                    return not TSBT.db.profile.spamControl.merging.enabled
                end,
                get = function()
                    return TSBT.db.profile.spamControl.merging.window
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.merging.window = val
                end
            },
            mergeShowCount = {
                type = "toggle",
                ...
                get = function()
                    return TSBT.db.profile.spamControl.merging.showCount
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.merging.showCount = val
                end
            },
            ...
            -- Throttling
            ----------------------------------------------------------------
            headerThrottle = {type = "header", name = "Throttling", order = 10},
            minDamage = {
                type = "range",
                ...
                step = 25,
                get = function()
                    return TSBT.db.profile.spamControl.throttling.minDamage
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.throttling.minDamage = val
                end
            },
            minHealing = {
                type = "range",
                ...
                step = 25,
                get = function()
                    return TSBT.db.profile.spamControl.throttling.minHealing
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.throttling.minHealing = val
                end
            },
            hideAutoBelow = {
                type = "range",
                ...
                step = 25,
                get = function()
                    return TSBT.db.profile.spamControl.throttling.hideAutoBelow
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.throttling.hideAutoBelow = val
                end
            },
            ...
            suppressDummy = {
                ...
                order = 21,
                get = function()
                    return TSBT.db.profile.spamControl.suppressDummyDamage
                end,
                set = function(_, val)
                    TSBT.db.profile.spamControl.suppressDummyDamage = val
                end
            }
        }
    }
    ...
    return {
        ...
        args = {
            ...
            enabled = {
                ...
                order = 2,
                get = function()
                    return TSBT.db.profile.cooldowns.enabled
                end,
                set = function(_, val)
                    TSBT.db.profile.cooldowns.enabled = val
                end
            },
            scrollArea = {
                type = "select",
                ...
                disabled = function()
                    return not TSBT.db.profile.cooldowns.enabled
                end,
                get = function()
                    return TSBT.db.profile.cooldowns.scrollArea
                end,
                set = function(_, val)
                    TSBT.db.profile.cooldowns.scrollArea = val
                end
            },
            format = {
                type = "input",
                ...
                get = function()
                    return TSBT.db.profile.cooldowns.format
                end,
                set = function(_, val)
                    TSBT.db.profile.cooldowns.format = val
                end
            },
            ...
            sound = {
                ...
                order = 5,
                values = function()
                    return TSBT.BuildSoundDropdown()
                end,
                ...
                get = function()
                    return TSBT.db.profile.cooldowns.sound
                end,
                set = function(_, val)
                    TSBT.db.profile.cooldowns.sound = val
                end
            },
                ...
                set = function(_, val)
                    ...
                    if spellID then
                        ...
                        if name then
                            ...
                            Addon:Print("Now tracking: " .. name .. " (ID: " ..
                                            spellID .. ")")
                        else
                            TSBT.db.profile.cooldowns.tracked[spellID] = true
                            Addon:Print("Now tracking spell ID: " .. spellID ..
                                            " (name not found)")
                        end
                        cooldownSpellInput = ""
            ...
            addSpellButton = {
                ...
                width = "half",
                disabled = function()
                    if not TSBT.db.profile.cooldowns.enabled then
                        return true
                    end
            ...
            trackedListHeader = {
                ...
                name = function()
                    ...
                    return "\n|cFFFFFFFFCurrently tracking " .. count ..
                               " spell(s):|r"
                end,
                order = 30,
                fontSize = "medium"
            ...
            trackedListContainer = {
                ...
                hidden = function()
                    local count = 0
                    for _ in pairs(TSBT.db.profile.cooldowns.tracked) do
                        count = count + 1
                    end
                    return count == 0
                end,
...
StaticPopupDialogs["TRUESTRIKE_DELETE_SCROLLAREA"] = {
    ...
    OnAccept = function(_, areaName)
        if not areaName or not TSBT.db or not TSBT.db.profile or
        ...
        if TSBT.IsScrollAreasUnlocked and TSBT.IsScrollAreasUnlocked() and
            TSBT.RefreshScrollAreaFrames then
            TSBT.RefreshScrollAreaFrames()
        end
...
StaticPopupDialogs["TRUESTRIKE_DELETE_LAST_SCROLLAREA"] = {
    text = "You cannot have zero scroll areas.\n\nIf you delete '%s', TrueStrike will immediately create a new default scroll area.",
    button1 = "OK",
    button2 = "Cancel",
    OnAccept = function(_, areaName)
        if not areaName or not TSBT.db or not TSBT.db.profile or
        ...
        LibStub("AceConfigRegistry-3.0"):NotifyChange("TrueStrike")
        TSBT.Addon:Print("Created new default scroll area: " ..
                             tostring(newName))
    end,
    timeout = 0,
    whileDead = true,
            ...
            container.args["trackedSpell_" .. slot] = {
                ...
                name = function()
                    ...
                    if type(idKey) == "string" and idKey:match("^item:") then
                        ...
                        local name = SafeGetItemName(itemID) or
                                         ("Item #" .. tostring(itemID or "?"))
                        return
                            "  \226\128\162 " .. name .. "  |cFF888888(ID: " ..
                                tostring(itemID or "?") .. ")|r"
                    else
                        local spellID = idKey
                        local name = SafeGetSpellName(spellID) or
                                         ("Spell #" .. spellID)
                        return
                            "  \226\128\162 " .. name .. "  |cFF888888(ID: " ..
                                spellID .. ")|r"
            ...
            container.args["removeSpell_" .. slot] = {
                ...
                func = function()
                    ...
                    if idKey then
                        ...
                        if type(idKey) == "string" and idKey:match("^item:") then
                            local itemID = tonumber(idKey:match("^item:(%d+)$"))
                            name = SafeGetItemName(itemID) or
                                       ("Item #" .. tostring(itemID or "?"))
                        else
                            local spellID = idKey
                            name = SafeGetSpellName(spellID) or
                                       ("Spell #" .. spellID)
                        end
                        StaticPopup_Show("TRUESTRIKE_REMOVE_SPELL", name, nil,
                                         idKey)
                    end
                end
            }
...
function TSBT.BuildTab_Media()
    return {
        ...
        args = {
            ...
            -- Sound Events
            ----------------------------------------------------------------
            headerSounds = {type = "header", name = "Sound Events", order = 1},
            ...
            lowHealthSound = {
                ...
                -- No dialogControl — uses standard dropdown
                get = function()
                    return TSBT.db.profile.media.sounds.lowHealth
                end,
                set = function(_, val)
                    TSBT.db.profile.media.sounds.lowHealth = val
                end
            },
            testLowHealth = {
                type = "execute",
            ...
            cooldownReadySound = {
                ...
                -- No dialogControl — uses standard dropdown
                get = function()
                    return TSBT.db.profile.media.sounds.cooldownReady
                end,
                set = function(_, val)
                    TSBT.db.profile.media.sounds.cooldownReady = val
                end
            },
            testCooldownReady = {
                type = "execute",
    ...
    for i = 1, TEST_COUNT do
        ...
        C_Timer.After((i - 1) * STAGGER_SECONDS, function()
            ...
            if TSBT.DisplayText then
                ...
                TSBT.Addon:Print(
                    "|cFF4A9EFFTEST 12345|r ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [" ..
                        areaName .. "] (" .. i .. "/" .. TEST_COUNT .. ")")
            end
        end)
    end
end
...
function TSBT.BuildTab_Diagnostics()
    return {
        ...
        order = 9,
        args = {
            headerDebug = {type = "header", name = "Debug Output", order = 1},
            debugLevel = {
                type = "select",
                name = "Debug Level",
            ...
            captureEnabled = {
                ...
                order = 11,
                get = function()
                    return TSBT.db.profile.diagnostics.captureEnabled
                end,
                set = function(_, val)
                    TSBT.db.profile.diagnostics.captureEnabled = val
                end
            },
            maxEntries = {
                type = "range",
                ...
                step = 50,
                disabled = function()
                    return not TSBT.db.profile.diagnostics.captureEnabled
                end,
                get = function()
                    return TSBT.db.profile.diagnostics.maxEntries
                end,
                set = function(_, val)
                    TSBT.db.profile.diagnostics.maxEntries = val
                end
            },
            clearLog = {
                type = "execute",
            ...
            logCount = {
                type = "description",
                name = function()
                    local count = TSBT.db.profile.diagnostics.log and
                                      #TSBT.db.profile.diagnostics.log or 0
            ...
            versionInfo = {
                ...
                name = function()
                    return
                        "|cFF4A9EFFTrueStrike|r v" .. (TSBT.VERSION or "?") ..
                            "\nWoW Interface: " ..
                            (select(4, GetBuildInfo()) or "?")
                end,
                order = 21,
                fontSize = "medium"
            },
            ...
            pipelineStatus = {
                ...
                name = function()
                    ...
                    if not p then
                        return "Pipeline diagnostics not loaded."
                    end
                    return string.format(
                               "CTU Fired: %d\n" ..
                                   "canRoute PASS: %d  |  FAIL: %d\n" ..
                                   "RouteToDisplay calls: %d\n" ..
                                   "DisplayHealWithSecret calls: %d\n" ..
                                   "Gate BLOCKED: %d\n" .. "Last CTU type: %s\n" ..
                                   "Last spellID: %d\n" .. "Last family: %s\n" ..
                                   "Last gate result: %s", p.ctuFired,
                               p.canRouteTrue, p.canRouteFalse,
                               p.routeToDisplay, p.displayCalled, p.gateBlocked,
                               p.lastCtuType, p.lastSpellID, p.lastFamily,
                               p.lastGateResult)
                end
            },
            resetPipeline = {
                type = "execute",
                name = "Reset Counters",
                desc = "Zero all pipeline counters.",
                order = 32,
                func = function()
                    if TSBT.ResetPipelineDiag then
                        TSBT.ResetPipelineDiag()
                    end
                    LibStub("AceConfigRegistry-3.0"):NotifyChange("TrueStrike")
                end
            }
